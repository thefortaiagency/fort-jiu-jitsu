-- BJJ Belt Progression System for The Fort Jiu-Jitsu
...
CREATE TABLE IF NOT EXISTS belt_ranks (
  ...
  color_hex TEXT NOT NULL, -- for UI display
  gradient_start TEXT, -- gradient start color for visual appeal
  gradient_end TEXT, -- gradient end color
  sort_order INTEGER NOT NULL,
  is_kids_belt BOOLEAN DEFAULT false,
  min_age INTEGER,
  max_age INTEGER,
  typical_time_months INTEGER, -- average time at this belt
  max_stripes INTEGER DEFAULT 4,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Member belt history (tracks all promotions)
...
-- Add current belt fields to members table
DO $$
BEGIN
...
-- Seed adult belt ranks (16+ years)
...
-- Everyone can view belt ranks (public data)
...
-- Members can view their own belt history
...
-- Function to update member's current belt when history is added
CREATE OR REPLACE FUNCTION update_member_current_belt()
RETURNS TRIGGER AS $$
BEGIN
  -- Mark all previous belt history as not current for this member
...
-- Trigger to auto-update member's current belt
...
-- Function to get belt promotion eligibility
...
BEGIN
  ...
  SELECT
    m.id AS member_id,
    CASE
      WHEN mbh.promoted_at IS NULL THEN false -- No belt history
...
COMMENT ON TABLE belt_ranks IS 'BJJ belt ranking system for both adults and kids';
COMMENT ON TABLE member_belt_history IS 'Complete promotion history for all members';
COMMENT ON FUNCTION update_member_current_belt IS 'Auto-updates member current belt when new history entry is added';
COMMENT ON FUNCTION get_promotion_eligibility IS 'Calculates if member is eligible for promotion based on time and attendance';
COMMENT ON VIEW belt_statistics IS 'Aggregate statistics of belt distribution';
