...
// Kiosk PIN - change this to your desired PIN
const KIOSK_PIN = '1234';
const KIOSK_AUTH_KEY = 'fort-kiosk-auth';
...
export default function KioskPage() {
  ...
  useEffect(() => {
    const authToken = localStorage.getItem(KIOSK_AUTH_KEY);
  ...
  // Load members on mount when authenticated
  useEffect(() => {
    if (isAuthenticated) {
      loadMembers();
      loadRecentCheckIns();
  ...
  const handlePinSubmit = () => {
    if (pinInput === KIOSK_PIN) {
      localStorage.setItem(KIOSK_AUTH_KEY, 'authenticated');
      setIsAuthenticated(true);
      setPinError(false);
    } else {
      setPinError(true);
  ...
  }, [mode, isAuthenticated, startScanner, stopScanner]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white flex flex-col items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2">Kiosk Mode</h1>
            <p className="text-gray-400">Enter PIN to access check-in kiosk</p>
          </div>
          ...
          {pinError && (
            <p className="text-red-400 text-center mb-4">Incorrect PIN</p>
          )}
            ...
            {['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', ''].map((num, i) => (
              num ? (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="bg-white/10 hover:bg-white/20 border border-white/20 rounded-2xl p-6 text-2xl font-bold transition-all"
                >
                  {num}
                </motion.button>
              ) : (
          ...
          {/* Link to personal check-in */}
          <div className="mt-12 text-center">
            <p className="text-gray-500 text-sm mb-2">Not on the kiosk?</p>
            <Link href="/check-in" className="text-white/70 hover:text-white underline text-sm">
              Use personal check-in instead
            </Link>
          </div>
        </div>
      </div>
  ...
  // Full Kiosk Mode (after PIN authentication)
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white relative overflow-hidden">
      ...
      <header className="bg-black/50 backdrop-blur-lg border-b border-gray-800/50 px-4 py-4 md:py-6 sticky top-0 z-40 relative">
        <div className="max-w-5xl mx-auto">
          {/* Top row - Kiosk Mode label */}
          <p className="text-center text-gray-400 text-sm md:text-base mb-3">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-lg border-t border-gray-800/50 px-4 py-4 z-20">
        <div className="max-w-5xl mx-auto flex items-center justify-between text-sm md:text-base text-gray-500">
          ...
          <span className="text-gray-600 hidden md:block">Tap your name or scan your code</span>
          <button
